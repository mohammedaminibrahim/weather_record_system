                    ...
                    <div class="col-lg-12 col-sm-6">
                        ...
                        if(isset($_POST['submit'])){
                          ...
                          if(!empty($stationname) && !empty($stationdistrict) && !empty($stationregion)){
                           ...
                           $sqlStation = "SELECT * FROM stations WHERE id = '$editid'";
                           $statement = $conn->prepare($sqlStation);
                           $results = $statement->execute();
                           $rows = $statement->rowCount();
                           $columns = $statement->fetchAll();

                           foreach($columns as $column){
                                $stationnameVal = $column['stationname'];
                                $stationdistrictVal = $column['stationdistrict'];
                                $stationregionVal = $column['stationregion'];
                           }

                          
                         
                            $sqlUpdateStation = "UPDATE stations SET  stationname = '$stationid' AND stationdistrict = '$stationdistrict' AND stationregion = '$stationregion')";
                      ...
                      <div class="tab-pane p-3 active preview" role="tabpanel" id="preview-1054">
                        <form action="" method="POST" class="row g-3">
                          <div class="col-md-3">
                            ...
                            <input name="stationname" value="<?=$stationnameVal;?>" class="form-control"  aria-disabled="true"  id="validationServer01" type="text" required="">
                          </div>

                          <div class="col-md-3">
                            <label class="form-label disabled" for="validationServer01">District Name</label>
                            <input name="stationdistrict" value="<?=$stationdistrictVal;?>" class="form-control"  aria-disabled="true"  id="validationServer01" type="text" required="">
                          </div>
